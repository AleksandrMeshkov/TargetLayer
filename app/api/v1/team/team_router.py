...
from app.services.team_service.delete_team_roadmap import delete_team_roadmap as delete_team_roadmap_service
...
@router.delete(
	"/{team_id}/roadmaps/{roadmap_id}",
	response_model=dict,
	openapi_extra={"security": [{"Bearer": []}]},
)
async def remove_team_roadmap(
	team_id: int = Path(..., gt=0),
	roadmap_id: int = Path(..., gt=0),
	current_user: User = Security(get_current_user),
	db: AsyncSession = Depends(get_db),
) -> dict:
	result = await delete_team_roadmap_service(db, current_user, team_id, roadmap_id)
	return result
